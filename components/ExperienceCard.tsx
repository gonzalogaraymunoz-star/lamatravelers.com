...
import { fallbackImage, productDuration, publicGroup } from '@/lib/catalog';
  ...
  return (
    <Link href={`/experiencias/${product.product_slug}`} className="experience-card">
      <div className="card-image-wrap">
        <img src={product.cover?.url || fallbackImage} alt={product.name} className="card-image" loading="lazy" />
      </div>
      <div className="card-body">
        <div className="eyebrow">{publicGroup(product)} · {productDuration(product)}</div>
        <h3>{product.name}</h3>
        <p>{product.description || 'Una experiencia coordinada por LAMA en San Pedro de Atacama.'}</p>
